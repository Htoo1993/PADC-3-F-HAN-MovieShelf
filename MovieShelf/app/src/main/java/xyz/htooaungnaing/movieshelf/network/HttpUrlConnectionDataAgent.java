package xyz.htooaungnaing.movieshelf.network;
...
import xyz.htooaungnaing.movieshelf.network.responses.GetMoviesResponse;
...
public class HttpUrlConnectionDataAgent implements MovieDataAgent {
    ...
    public void loadMovies() {
        Log.d(MovieShelfApp.LOG_TAG,"load in main thread");

        new AsyncTask<Void, Void, Void>(){

            @Override
            protected Void doInBackground(Void... voids) {
                Log.d(MovieShelfApp.LOG_TAG,"load in background thread");
                ...
                try {
                    ...
                    Log.d(MovieShelfApp.LOG_TAG, "responseString : " + responseString);

                    Gson gson =new Gson();
                    GetMoviesResponse getMoviesResponses = gson.fromJson(responseString, GetMoviesResponse.class);
                    Log.d(MovieShelfApp.LOG_TAG,"Popular movie list size : " + getMoviesResponses.getMovieList().size());

                    EventBus.getDefault().post(new LoadedMoviesEvent(getMoviesResponses.getMovieList()));

                } catch (Exception e) {
                    Log.e(MovieShelfApp.LOG_TAG, e.getMessage());
                    /*
